<template>
    ...
    <div class="sponsorsContainer">
        <div class="miniCon" v-for="(col, i) in sponsorColumns" :key="i">
            <span v-for="name in col" :key="name">{{ name }}</span>
        </div>
    </div>
</template>
...
export default {
    ...
    data() {
        return {
            ...
            gakuenmenkyoText: "",
            harunohiText: "",
            sponsors: thanksData.sponsors
        }
    },
    computed: {
        sponsorColumns() {
            const perCol = Math.ceil(this.sponsors.length / 4)
            return Array.from({ length: 4 }, (_, i) =>
                this.sponsors.slice(i * perCol, (i + 1) * perCol)
            )
        }
    }
}
